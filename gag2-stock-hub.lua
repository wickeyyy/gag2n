...
local function getTopColor(items)
    local topColor = RI.Common
    for _, item in ipairs(items) do
        for _, tier in ipairs(TIER_ORDER) do
            if item.rarity == tier then
                topColor = RI[tier] or topColor
                break
            end
        end
    end
...
tl("Auto Reporter  v4",10,C.Sub,28,23)
...
-- Force send all button
local forceBtn=Instance.new("TextButton",SB)
forceBtn.Size=UDim2.new(1,-8,0,30) forceBtn.BackgroundColor3=Color3.fromRGB(80,40,160)
forceBtn.Text="🚀 Force Send All" forceBtn.TextColor3=Color3.new(1,1,1)
forceBtn.TextSize=10 forceBtn.Font=Enum.Font.GothamBold forceBtn.BorderSizePixel=0
forceBtn.LayoutOrder=12
...
local function makeItemRow(parent, item, index)
    ...
    dot.BackgroundColor3=item.inStock and C.Green or C.Red dot.BorderSizePixel=0
    Instance.new("UICorner",dot).CornerRadius=UDim.new(1,0)

    local nameLbl=Instance.new("TextLabel",row)
    nameLbl.Size=UDim2.new(1,-155,1,0) nameLbl.Position=UDim2.new(0,20,0,0)
    nameLbl.BackgroundTransparency=1
    nameLbl.Text=(RE[r] or "⚪").." "..item.name
    ...
    stockLbl.Text=item.inStock and item.stock or "No Stock"
    stockLbl.TextColor3=item.inStock and C.Green or C.Red
...
local propPage, propSc, propStats, propSend, propScan  = buildShopPage("Props","🪨")

-- ── Shop Scanner (fixed: skip ItemTemplate) ───────────────────────────────────
    ...
    pcall(function()
        ...
        for _, v in pairs(shopGui:GetDescendants()) do
            if v:IsA("TextLabel") and v.Name == "Seed_Text" then
                -- Skip ItemTemplate (it's a dummy/placeholder)
                ...
                local inStock = not stock:find("NO STOCK") and not stock:find("x0 in Stock") and stock ~= ""
...
local function updateDisplay(sc, statsLbl, items)
    ...
    local inCount = 0
    -- Sort: in stock first, then by rarity tier
    local sorted = {}
    ...
    statsLbl.Text = "✅ "..inCount.." in stock  ·  "..#items.." total"
    statsLbl.TextColor3 = inCount > 0 and C.Green or C.Sub
end

-- ── Build combined Discord payload (Seeds + Gear + Props in one message) ───────
local function buildCombinedPayload(seedItems, gearItems, propItems, weatherList)
    local embeds = {}
    local hasAnyRare = false
    ...
    local function shopEmbed(shopName, icon, items)
        ...
        for _, item in ipairs(items) do
            if item.inStock then
                ...
                if RARE_TIERS[item.rarity] then hasRare=true hasAnyRare=true end
                for _, tier in ipairs(TIER_ORDER) do
        ...
        if #inStock == 0 then
            table.insert(embeds, {
                title=icon.." "..shopName,
                description="No items currently in stock",
                color=RI.Common,
        ...
        table.insert(embeds, {
            title=icon.." "..shopName.." ("..#inStock.." in stock)",
    ...
    return HttpService:JSONEncode({
        content=hasAnyRare and "🚨 **Rare item in Grow A Garden 2!**" or nil,
        embeds=embeds,
    })
end
...
local function runFullScan(force)
    ...
    if #newProps > 0 then propItems = newProps  updateDisplay(propSc, propStats, propItems) end

    if WEBHOOK_URL == "" then
        lastScanLbl.Text = "⚠️ No webhook set"
        return
    end
    ...
    for _, item in ipairs(seedItems) do
        snapshot["s:"..item.name] = item.stock
        if lastStockSnapshot["s:"..item.name] ~= item.stock then changed=true end
    end
    for _, item in ipairs(gearItems) do
        snapshot["g:"..item.name] = item.stock
        if lastStockSnapshot["g:"..item.name] ~= item.stock then changed=true end
    end
    for _, item in ipairs(propItems) do
        snapshot["p:"..item.name] = item.stock
        if lastStockSnapshot["p:"..item.name] ~= item.stock then changed=true end
    end

    -- Weather pings for new weather
    local wxSnapshot = {}
    ...
    for k in pairs(lastWeather) do if not wxSnapshot[k] then changed=true break end end

    -- Send combined message if changed
    if changed then
        local payload = buildCombinedPayload(seedItems, gearItems, propItems, weather)
...
propSend.MouseButton1Click:Connect(function() forceSendShop(propItems,"Props","🪨",propSend,"📤  Force Send Props to Discord") end)

seedScan.MouseButton1Click:Connect(function()
    seedScan.Text="🔍 Scanning..."
    task.spawn(function() local i=scanShop("SeedShop") if #i>0 then seedItems=i updateDisplay(seedSc,seedStats,i) end seedScan.Text="✅ Done!" task.delay(2,function() seedScan.Text="🔍  Refresh Now" end) end)
end)
gearScan.MouseButton1Click:Connect(function()
    gearScan.Text="🔍 Scanning..."
    task.spawn(function() local i=scanShop("GearShop") if #i>0 then gearItems=i updateDisplay(gearSc,gearStats,i) end gearScan.Text="✅ Done!" task.delay(2,function() gearScan.Text="🔍  Refresh Now" end) end)
end)
propScan.MouseButton1Click:Connect(function()
    propScan.Text="🔍 Scanning..."
    task.spawn(function() local i=scanShop("CrateShop") if #i>0 then propItems=i updateDisplay(propSc,propStats,i) end propScan.Text="✅ Done!" task.delay(2,function() propScan.Text="🔍  Refresh Now" end) end)
end)
...
local function refreshWeatherDisplay()
    ...
    for name,btn in pairs(wxIconRefs) do
        if activeNames[name] then
            btn.BackgroundColor3=RARITY_BG.Legendary or C.Card
...
wInfo.Size=UDim2.new(1,0,0,130) wInfo.Position=UDim2.new(0,0,0,230) wInfo.BackgroundTransparency=1
wInfo.Text="Webhook is saved locally and auto-loaded every time you run the script — no need to paste it again!\n\nHow to get a webhook:\n1. Discord → channel → ⚙️ Edit → Integrations\n2. Webhooks → New Webhook → Copy URL\n3. Paste above → Save\n\nFor weather role pings:\nEdit WEATHER_ROLE_PINGS at the top of the script"
...
wTest.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        local ok,err=httpPost(WEBHOOK_URL,HttpService:JSONEncode({
            embeds={{title="✅ Grow A Garden 2 Stock — Test",description="Connected!\n🌱 Seeds · ⚙️ Gear · 🪨 Props · 🌤️ Weather",color=3066993,footer={text="Grow A Garden 2 Stock"}}}
        }))
...
local tabs={
    ...
    Props={propPage,propTab,propTabL}, Weather={WXP,wxTab,wxTabL},
...
forceBtn.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        lastStockSnapshot={} lastWeather={} sentWeatherPings={}
        runFullScan(true)
        forceBtn.Text="✅ Done!"
...
print("[Grow A Garden 2 Stock v4] Loaded!")
